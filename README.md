...
The pattern itself can come from either of two sources. Gradient Noise is computed on the spot and needs nothing on disk. Blue Noise Mask reads a stored spatiotemporal mask instead, which is spread more evenly across the screen and, unlike a scrolled pattern, also spreads each individual pixel's run of values over time — so the grain sits still rather than crawling when the camera holds. The mask is generated by `tools/make_stbn.py` using void-and-cluster in three dimensions, with an independent volume in each of red, green and blue so one fetch feeds all three channels. Gradient Noise is the default because it keeps the shader working with no extra files; the mask is the better pattern if you copy it in.
...
**Requires:** `ReShade.fxh` only, plus `Textures/dz_stbn_512x256.png` from this repo. Copy that file into your ReShade `Textures` folder along with the shader — ReShade will complain about the missing texture if it isn't there, and the Blue Noise Mask dither option won't work. No additional shader packs needed.
...
| Dither Pattern                 | Gradient Noise  | Where the pattern comes from. Gradient Noise needs no files. Blue Noise Mask is spread more evenly and settles rather than crawling, but needs the texture copied in. |
